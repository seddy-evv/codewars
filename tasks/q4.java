...
public class TimeFormatter {
    ...
    public static String formatDuration(int sec) {
        ...
        String[] keys = {"year", "day", "hour", "minute", "second"};
        duration.put("year", sec / 31536000);
        duration.put("day", (sec % 31536000) / 86400);
        duration.put("hour", ((sec % 31536000) % 86400) / 3600);
        duration.put("minute", (((sec % 31536000) % 86400) % 3600) / 60);
        duration.put("second", (((sec % 31536000) % 86400) % 3600) % 60);
        String output = "";

        for (String i : keys) {
            if (duration.get(i) > 0 && duration.get(i) == 1){
                output += " " + duration.get(i) + " " + i + ",";
            } else if (duration.get(i) > 0){
                output += " " + duration.get(i) + " " + i +"s" +  ",";
            }
        }
        StringBuilder outputFormat = new StringBuilder(output);
        outputFormat.setCharAt(output.lastIndexOf(","), ' ');
        output = outputFormat.toString().trim();
        if (output.lastIndexOf(",") > 0) {
            output = output.substring(0, output.lastIndexOf(",")) + " and" + output.substring(output.lastIndexOf(",") + 1, output.length());
        }
        return output;
    }
}